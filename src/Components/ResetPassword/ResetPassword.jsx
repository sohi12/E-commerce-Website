...
export default function ResetPassword() {
    const [isLoading , setIsLoading] = useState(false);
      ...
      const {values , handleSubmit , handleChange , touched , handleBlur , errors , isValid, dirty} = useFormik({
        initialValues:{
            email : '',
            newPassword : ''
        },
     ...
     console.log(isValid, dirty) 
      return (
        <form onSubmit={handleSubmit} className='w-75 m-auto my-5' >
            <label className='' htmlFor="email">Please Enter Your Email :</label>
            <input value={values.email} onChange={handleChange} onBlur={handleBlur} type="email" className='form-control my-3' id='email' name='email' />
            ...
            <input value={values.newPassword} onChange={handleChange} onBlur={handleBlur} type="password" className='form-control my-3' id='newPassword' name='newPassword'autoComplete='on'/>
            ...
            :
            <button disabled={(!dirty&&isValid) } type='submit' className='btn bg-main px-3 text-white ms-auto d-block'>Send</button>
            }
        </form>
      )
 }
